...
-   **ゲームシステム (`specification.md`)**
    -   [x] **魔法システム:**
        ...
        -   [x] **範囲ダメージの実装**
            -   魔法が敵に着弾した際、`damageRangeOnHit` の範囲内にいる敵にダメージを与えるように修正。
            -   貫通弾が敵にヒットした際にも、その都度、範囲ダメージが発生するように修正。
            -   `isSelfDamage` が `true` の場合、範囲ダメージがプレイヤーにも適用されるように修正。
        -   [x] **範囲ダメージの視覚エフェクト実装**
            -   範囲ダメージ発生時に、魔法の属性に応じたパーティクルエフェクトが表示されるように実装。
            -   エフェクト定義 (`kigame/src/data/effects.ts`)、汎用コンポーネント (`kigame/src/components/ParticleEffect.tsx`)、CSS (`kigame/src/styles/effects.css`) を新規作成し、再利用性とメンテナンス性を考慮した設計にした。
            -   各属性（打撃、炎、氷、雷、切断）ごとに、特徴的なパーティクルアニメーションを実装。
            -   パーティクルエフェクトが範囲ダメージの範囲に表示されるように修正。
        -   [x] **地面効果の自傷設定**
            -   地面効果（`GroundEffect`）自体に `isSelfDamage` プロパティを追加し、`kigame/src/data/spells.ts` で個別に設定できるように修正。
            -   `fireball` の地面効果でプレイヤーがダメージを受けないように修正。
    -   [x] **敵システム:**
